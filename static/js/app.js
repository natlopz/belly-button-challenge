...
function buildMetadata(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    if (result) {
      ...
      // If no metadata was found for the sample, display a message
      PANEL.append("p").text("No metadata found for this sample.");
    }
  }).catch(error => {
    console.error("Error fetching or processing data:", error);
    // Display an error message in the metadata panel if an error occurs
    PANEL.html("Error loading metadata.");
  });
}
